...
function DishModal({ isOpen, onOpen, onOpenChange, id, refresh }: ModalProps) {
	const styleArea = {
	...
	const dishServerice = useDishService();

	const submit = async (onClose: () => void) => {
		if (name === "" || price === "" || (category !== "all" && category.size == 0)) {
		...
		try {
			...
			var params: any = {name, categoryId: categoryMap[Array.from(category)[0]], price: Number(price), description}
			...
			if (id) {
				params = {...params, id: id};
				resp = await dishServerice.update(params);
			} else {
				params = {...params, status: 1};
				resp = await dishServerice.add(params);
			}
	...
	const fetchDish = async () => {
		if (!id) return;
		try {
			const resp = await dishServerice.getById(id);
		...
		<div className='flex items-center justify-center text-[#e0e0e0] bg-[#1b2537]'>
			<M isOpen={isOpen} onOpenChange={onOpenChange}
				...
				<ModalContent>
					{(onClose) => (
						<div>
							<ModalHeader className="flex flex-col gap-1 pb-0">{id ? 'Update Dish' : 'Add Dish'}</ModalHeader>
							...
							<ModalBody>
								...
								<div className="flex items-center justify-center w-full">
									<label htmlFor="dropzone-file" className="flex flex-col items-center justify-center w-full h-64 border-1 border-[#e0e0e0] rounded-lg cursor-pointer bg-[#1b2537]">
										{imgUrl ? <img onLoad={() => URL.revokeObjectURL(imgUrl)}
										src={imgUrl} className='h-64 w-64 m-[2px]' /> :
